...
func _process(_delta: float) -> void:
	if not _enabled or not _time_log_enabled:
		return
	_log_check_tick()
...
func _on_instance_stage_changed(instance, new_stage: int) -> void:
	if not _eneano_log_enabled:
		return
	var name: String = instance.enemy_name if "enemy_name" in instance else "?"
	print("DebugTools[EneAno]: STAGE_CHANGED[%s] -> %s  pos=%s  t=%.0fs" % [
		name, new_stage, instance.global_position, GlobalTimeManager.elapsed_time
	])
